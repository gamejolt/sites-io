...
if ( isProd ) {
	...
	renderer = createRenderer( fs.readFileSync( resolve( '../build/dev-server/app.server.js' ), 'utf-8' ) );
	indexHTML = parseIndex( fs.readFileSync( resolve('../build/dev/index.html' ), 'utf-8' ) );
}
else {
...
app.get( '*', ( req, res ) =>
{
	...
	console.log( renderer.renderToString( context, ( err ) =>
	{
		if ( err ) {
			console.error( err );
		}
